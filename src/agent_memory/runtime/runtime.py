...
from agent_memory.policy import DifferentiatedPolicy, OverWindowPlan, WindowProcessPlan
from agent_memory.query_schema import output_columns
from agent_memory.window import (
    WINDOW_SOURCE_INPUT,
...
class MemoryRuntime:
    ...
    def _process_completed_windows(
        ...
        for window in windows:
            result = self.adapter.execute(
                plan.process_query,
                self._window_process_inputs(window.frame, plan=plan),
            )
    ...
    def _process_upstream_plan(
        ...
        if (
            ...
            current = self._state.get(
                upstream_name,
                self._empty_materialized_frame(
                    upstream_name,
                    upstream_query,
                    source=self._state.get("log"),
                ),
            )
            full_inputs = self._maintenance_inputs(
                changed_rows,
                extra={
                    "log": self._state.get("log", changed_rows),
                    upstream_name: current,
                },
            )
    ...
    def _window_process_inputs(
        self,
        window_frame: Any,
        plan: WindowProcessPlan | None = None,
    ) -> dict[str, Any]:
        """Build adapter inputs for executing one window process body.

        When a WindowProcessPlan is provided, injects previous_entries from
        the private accumulator so the process body can cross-reference prior
        windows for deduplication.
        """

        import pandas as pd

        inputs = dict(self._state)
        frame = window_frame.copy() if hasattr(window_frame, "copy") else window_frame

        if plan is not None:
            private = self._state.get(plan.private_name, pd.DataFrame())
            if not private.empty and "lossless_restatement" in private.columns:
                last_entries = private["lossless_restatement"].tail(3).tolist()
                if last_entries:
                    previous_text = "[Previous Window Memory Entries (for reference to avoid duplication)]\n"
                    previous_text += "\n".join(f"- {e}" for e in last_entries)
                else:
                    previous_text = ""
            else:
                previous_text = ""
            frame["previous_entries"] = previous_text

        inputs[WINDOW_SOURCE_INPUT] = frame
        return inputs
    ...
    def _empty_materialized_frame(
        ...
        *,
        ...
        if source is not None and hasattr(source, "columns"):
            return pd.DataFrame(columns=list(source.columns))
        columns: list[str] = []
        for node in self._walk_query(query):
            if node.op == "materialized_view" and node.params.get("name") == name:
                ...
                break
        if not columns and query.op == "log":
            columns = [column.name for column in query.params.get("columns", ())]
        return pd.DataFrame(columns=columns)

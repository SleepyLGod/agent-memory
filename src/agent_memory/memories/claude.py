...
Each output row should correspond to one memory file.
...
_GROUP_INSTRUCTION = """
Rows refer to the same durable Claude memory topic when their {name},
...
You are performing a dream — a reflective pass over Claude memory candidate rows. Synthesize what you've learned recently into durable, well-organized memories so that future sessions can orient quickly.

## Phase 3 — Consolidate

For each thing worth remembering, produce or update one canonical memory row. Use the memory file format and type conventions from the auto-memory section — it's the source of truth for what to save, how to structure it, and what NOT to save.

Focus on:
- Merging new signal into existing topic rows rather than creating near-duplicates
- Converting relative dates ("yesterday", "last week") to absolute dates so they remain interpretable after time passes
- Deleting contradicted facts — if today's evidence disproves an old memory, fix it at the source
- Update or remove information that is stale, wrong, or superseded
- Resolve contradictions — if two candidate rows disagree, fix the wrong one
    ...
    topics = (
        ...
        .sem_flat_map(
            ...
            output_cols={
                ...
                "type": "Claude memory type: user, feedback, project, or reference.",
                "body": "Candidate markdown memory body.",
            },
        ...
        .sem_agg(
            ...
            output_cols={
                ...
                "type": "Claude memory type: user, feedback, project, or reference.",
                "body": "Consolidated markdown memory body.",
            },

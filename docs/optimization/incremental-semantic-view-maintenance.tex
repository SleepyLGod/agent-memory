...
The current public API treats semantic group-by as semantic grouping plus an
...
Here \texttt{key} names the columns used as the grouping basis, the instruction
defines semantic membership, and \texttt{agg} produces the output row for each
group. The aggregate can be ordinary, such as \texttt{count()} or
\texttt{max("timestamp")}, or semantic, such as \texttt{sem\_agg(...)}.
...
delta_groups = delta_topic_candidates.sem_groupby(
    key=["topic_name"],
    instruction="Find candidates related to the same durable memory topic.",
    agg=sem_agg(
        input_cols=["topic_name", "topic_content"],
...
entities = entity_candidates.sem_groupby(
    key=["entity_name"],
...
facts = fact_candidates.sem_groupby(
    key=["fact", "subject"],
